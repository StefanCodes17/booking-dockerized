<template>
  <div class="max-w-7xl mx-auto py-6">
    <div class="flex justify-between">
      <h1 class="text-3xl font-bold text-gray-900">Стаи</h1>
      <router-link
        to="/rooms/create"
          ...
          <table class="min-w-full">
            <thead class="border-b bg-gray-600">
              <tr>
                ...
                <th scope="col" class="text-sm font-medium text-white px-6 py-4 text-left">Име</th>
                <th scope="col" class="text-sm font-medium text-white px-6 py-4 text-left">Цена</th>
                <th scope="col" class="text-sm font-medium text-white px-6 py-4 text-left">
                  Номер
                </th>
                <th scope="col" class="text-sm font-medium text-white px-6 py-4 text-left">Хора</th>
                <th class="text-sm font-medium text-white px-6 py-4 text-left"></th>
              </tr>
            </thead>
            <tbody>
              <tr class="border-b" v-for="(room, key) in rooms" :key="key">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  {{ room.id }}
                </td>
                <td class="text-sm font-light px-6 py-4 whitespace-nowrap">
                  <img :src="room.image" alt="" class="rounded-lg w-20 h-full" />
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  {{ room.name }}
                </td>
